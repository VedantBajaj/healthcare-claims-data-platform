...
daily_inpatient_claims as (
    ...
    from {{ ref('stg_daily_inpatient_claims') }}
),

daily_outpatient_claims as (
    select
        beneficiary_id,
        claim_id,
        claim_segment,
    ...
    from {{ ref('stg_daily_outpatient_claims') }}
),
...
unioned_claims as (
    ...
    union all
    select * from daily_inpatient_claims
    union all
    select * from daily_outpatient_claims
),

valid_claims as (
    select *
    from unioned_claims
    where nullif(trim(beneficiary_id), '') is not null
